...
#     BrainScale4BCaF1REV.R
#     /DS/RhDec20/BrainScale4/newHippoScale
#      October 7, 2021
#
#  The REVision is that the gene list used is the
#  FDR=0.10 list from the DEG analysis of the Science paper samples
#
...
#
#   Nov 17, 2021 -- fix boxplot titling
...
in5DataPath="/DS/RhDec20/BrainScale4/newHippoScale/"
...
# == == BRING IN THE GENE LIST FROM THE SCIENCE PAPER == === =

fn=paste0(in5DataPath,"DEGsForHippoScale.RData")
load(fn)

sum(DEG10ForHippoScale %in% rownames(expT)) # 732--good



expT.both=expT[DEG10ForHippoScale,]
   

# ====== PCA Approach =================

# -- PCA on TC & PTC to get PC1 for scale
...
# = = = MAKE THE PCA PLOT SHOWING TC & PTC SEPARATION ON PC1  = = =

pca1=pca(expT.both.TCPTC,
         metadata = mT.TCPTC,
         removeVar = 0,scale=T,center=T)

tl=paste0("Hippocampus data (BCaF1), TC & PTC samples")
st=paste0("Genes from DEG results (FDR=0.10) of macaque data from Zhu et al., 2018")
screeplot(pca1,title = tl,subtitle = st)

# *** Reverse PC1 loadings and values for more intuitive direction

pca1$rotated$PC1=-1 * pca1$rotated$PC1
...
# Reverse direction on PC1 so more intuitive
MB$PC1=-1*MB$PC1

tl=paste0("Hippocampus data (BCaF1), all samples, PCA-based scoring")
st=paste0("Genes from DEG results (FDR=0.10) of macaque data from Zhu et al., 2018")

href=median(MB$PC1[MB$TrtShort=="PTC"])
scoring.bp=ggplot(MB,aes(x=TrtShortF,y=PC1)) +
   geom_boxplot(outlier.shape = NA,color='gray50') +
   geom_jitter(width=0.2,height=0,color='dodgerblue3') +
   labs(title=tl,subtitle = st,y='Hippocampus RNA Maturity Score') +
   geom_hline(yintercept = href,color="black",
              lty="dotted") 
scoring.bp
scoring.bp=scoring.bp +
   coord_flip() +
   scale_x_discrete(limits=rev)
AddThemes(scoring.bp)

#  looks OK, B060 highest Tx

# ***  SAVE pca.obj & MB for other work ***
fn=paste0(outDataPath,"BrainScalePCAstuffREV.RData")
Brain_TCvPTC_pca.obj=pca.obj
Brain_ScoreAllSamples=MB
Brain_Gene.list=DEG10ForHippoScale
save(file = fn,Brain_TCvPTC_pca.obj,Brain_Gene.list,
     Brain_ScoreAllSamples)

#############  BELOW NOT REDONE OR RERUN  ##################
...
AddThemes=function(GGplot,Tcolor="black",STcolor="black")
{
   #  function adds title, subtitle and axis themes
   GGplot = GGplot +
      theme(axis.text.x = element_text(face="bold", size=12, angle=0,vjust=1,hjust=1),
            axis.text.y = element_text(face="bold",size=12)) +
      theme(plot.title = element_text(color=Tcolor, face="bold.italic", size=15, hjust=0)) +
      theme(axis.title = element_text(color="black", face="bold", size=13)) +
      theme(plot.subtitle = element_text(color=STcolor, size=12, 
                                         face="italic",hjust=0))  
